...
data "aws_ssm_parameter" "transaction_sns_topic_arn" {
  ...
  policy    = <<POLICY
  {
      ...
      "Statement": [
        ...
        },
        {
            ...
            "Condition": {
                "ArnEquals": {
                    "aws:SourceArn": "${data.aws_ssm_parameter.transaction_sns_topic_arn.value}"
                }
            }
        },
        {
...
resource "aws_sns_topic_subscription" "housing_search_listener_queue_subscribe_to_transaction_sns" {
  topic_arn            = data.aws_ssm_parameter.transaction_sns_topic_arn.value
  protocol             = "sqs"
  endpoint             = aws_sqs_queue.housing_search_listener_queue.arn
  raw_message_delivery = true
}
