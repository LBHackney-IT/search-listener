...
namespace HousingSearchListener.V1.Domain.Tenure
{
    ...
    public static class TenureTypes
    {
        ...
        public static string GetPersonTenureType(string tenureTypeCode, bool isResponsible)
        {
            if (!isResponsible)
            {
                if (tenureTypeCode == TenantGarage.Code) return "Occupant";
                else return "HouseholdMember";
            }
            else
            {
                if (FreeholderCodes.Contains(tenureTypeCode)) return "Freeholder";
                if (LeaseholderCodes.Contains(tenureTypeCode)) return "Leaseholder";
                else return "Tenant";
            }
        }
    }
